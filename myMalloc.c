 ...
 #include "printing.h"
  ...
  //Additional helper functions I have made
  ...
  static inline void update(header * old_hdr, header * new_hdr, size_t block_size);
...
*/
 ...
 static inline header * allocate_object(size_t raw_size) {
  ...
  //calculates the required block size. If the raw_size is larger than 2* the size of a normal block then
   //raw_size += the header size. Otherwise we just use the header size.
 ...
 //Helper function that returns a pointer to the free list
 //Takes the block size as a parameter
 static inline header * ptr_to_freelist(size_t block_size) {
   int index = (int) (block_size - ALLOC_HEADER_SIZE) / sizeof(size_t) - 1;
 ...
 //separates the first block and places it into the free list
 static inline header * split(header * cur, size_t block_size) {
 ...
 //inserts a block into the right free list
 static inline void insert_into_freelist(header * hdr) {
   int index = (get_size(hdr)-ALLOC_HEADER_SIZE)/sizeof(size_t)-1;
 ...
 static inline void deallocate_object(void * p) {
   ...
   if (get_state(rightblock) == UNALLOCATED &
       ...
       if (last_freelist(leftblock)) {
         ...
         delete(leftblock);
         update(rightblock, leftblock, new_size);
       } else {
         delete(leftblock);
         delete(rightblock);
        set_size(leftblock, new_size);
    ...
    if (last_freelist(rightblock)) {
      update(rightblock, hdr, get_size(hdr) + get_size(rightblock));
    } else {
      delete(rightblock);
